...
DEFAULT_CORPUS_PATH = Path("data/sources/dockets_corpus.json")
SOURCE_NAME = "dockets"
...
def _load_corpus(corpus_path: Path) -> List[Dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError("dockets corpus must be a JSON array")
    return payload


def run_dockets_source_agent(agent_input: Dict[str, Any]) -> Dict[str, Any]:
    request = agent_input.get("request", {})
    source_name = str(request.get("source_name", "")).lower()
    if source_name != SOURCE_NAME:
        raise ValueError("source_name must be 'dockets'")
        ...
        scored.append(
            {
                ...
                "snippet": str(row.get("summary", ""))[:400],
                "relevance": round(min(1.0, 0.35 + base_score), 3),
                "source_type": "docket",
                "jurisdiction": str(row.get("jurisdiction", "")),
                "decision_date": str(row.get("decision_date", "")) or None,
                "court_level": str(row.get("court", "")) or None,
                "precedential_status": "non-precedential",
                "citation_count": None,
